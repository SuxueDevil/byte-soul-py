...
三个加载器的加载策略：
- UnstructuredMarkdownLoader: 按 Markdown 语义结构（标题、段落、列表）自动分段
- PyPDFLoader: 按页加载，每页一个 Document
- Docx2txtLoader: 提取 Word 全文文本，整篇作为一个 Document
"""
...
class Loader:
    ...
    def markdown(file_path: str) -> list[Document]:
        ...
        Returns:
            Document 列表（按语义结构拆分：标题、段落、列表各成一块）
        """
    ...
    def pdf(file_path: str) -> list[Document]:
        ...
        Returns:
            Document 列表（按页拆分，每页一个 Document）
        """
